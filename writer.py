...
class Writer:
    def __init__(
        self, profileInfos: dict, links: List[str], missingLinks: List[str]
    ) -> None:
        self.profileInfos = profileInfos
        self.links = links
        self.missingLinks = missingLinks
        self.VISITED_LINKS_FILE_PATH = "PATH_TO_VISITED_LINKS_FILE"
        self.CSV_FOLDER_PATH = "PATH_TO_CSV_FILE"
        self.MISSING_LINKS_FOLDER_PATH = "PATH_MISSING_LINKS_FILE"
    ...
    def writeProfileData(self) -> None:
        ...
        filePath = self.getFileName(fileName=self.CSV_FOLDER_PATH)
    ...
    def writeMissingLinksData(self) -> None:
        if len(self.missingLinks) == 0:
            return
        filePath = self.getFileName(fileName=self.MISSING_LINKS_FOLDER_PATH)
        fd = open(filePath, "w")
